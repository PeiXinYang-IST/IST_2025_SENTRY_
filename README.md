...
## /* 2025 1.2 */
实现了局部点云分割，但效果并不理想，尤其体现在x和y轴上，z轴分割和实时点云滤波保持一致 后续查看
修改了fast_planner中对于起点的判定为move_base的全局规划起点  

测试：
kinodynamic A*的全局搜索目前在实体中优于拓扑规划，但在仿真中则是topu更加有优势
...
## /* 2025 1.11 */
添加了寻优的并行处理，加快初始化重定位速度
TODO：

雷达对障碍物点云进行聚类，获得大致yaw 进行预瞄 （添加kalman?)

## /* 2025 1.14 */
完成对docker的环境部署，在ubuntu 22.04上运行成功
使用TCP网络通信与视觉对接成功
...
## /* 2025 3.14 */
对fast planner的前端进行改进 实现全局规划成功率 95%以上但是在动态障碍物过多的情况下会出现维度灾难情况
考虑使用其他方案 

## /*2025 4.8 */
使用GCOPTER  （informed RRT*+安全走廊+MINCO） 基于采样的前端路径相比与基于图搜索的方法更适合安全走廊生成  
修改使用GCOPTER思路：
1.实现使用里程计实时规划 路线
