...
void cloudCallback(const sensor_msgs::PointCloud2ConstPtr& input_cloud_msg) {
    ...
    pass.setFilterLimits(clear_distance_x-0.6, clear_distance_x+0.6); // x轴半径限制
    pass.filter(*cloud_filtered_cylinder);

    pass.setInputCloud(cloud_filtered_cylinder);
    pass.setFilterFieldName("y");
    pass.setFilterLimits(clear_distance_y-0.6, clear_distance_y+0.6); // y轴半径限制
    pass.filter(*cloud_filtered_cylinder);
...
for (size_t i = 0; i < cloud->points.size(); ++i) {
    ...
    pass_z.setFilterLimits(-std::numeric_limits<float>::max(),clear_distance_z); // 低于雷达z坐标的点云滤除
...
for (size_t i = 0; i < cloud->points.size(); ++i) {
    ...
    pass_z.setFilterFieldName("z");
    pass_z.setFilterLimits(clear_distance_z+2.0,std::numeric_limits<float>::max()); // 距离雷达z坐标过高的点云滤除
...
for (size_t i = 0; i < cloud->points.size(); ++i) {
    ...
    pcl::toROSMsg(*cloud_filtered_cylinder_removed_z_2, output_cloud_msg);
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "sim_pcl_processing_node");
    ros::NodeHandle nh;
    ...
    // 创建Subscriber来订阅原始点云
    sub = nh.subscribe("point_cloud_map", 1, cloudCallback);
